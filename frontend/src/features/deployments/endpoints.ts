...
/**
 * The shared host suffix behind a sample URL: `edc-sample.example.org` carries
 * the connector's own name as its first segment, so everything after the first
 * dash is what other connectors share. Used only when the backend does not
 * state the suffix outright.
 */
export function deriveHostnameSuffix(value?: string) {
  if (!value) {
    return '';
  }
