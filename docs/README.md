...
This guide introduces the tutorial for the end-to-end data exchange within the Tractus-X ecosystem. The starting point is the Tractus-X Portal, which serves as the central environment for managing an organization and accessing essential tools. Within the portal, users will work with the EDC Management Console to provision (EMC) the required components, including the Eclipse Dataspace Connector and the Submodel Server.

Once these core services are deployed and configured, data exchange is carried out using the Simple Data Exchanger (SDE). The SDE provides a streamlined interface that enables organizations to share data with minimal complexity while adhering to the principles of secure, sovereign, and federated data spaces.
...
- Authentication via Keycloak: The console integrates with Keycloak for user authentication, ensuring secure access control and role-based permissions. 
